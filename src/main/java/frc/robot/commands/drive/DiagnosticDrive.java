...
public class DiagnosticDrive extends CommandBase {
    ...
    @Override
    public void execute() {
        double leftMaster = Math.abs(leftMasterSupplier.getAsDouble()) > kNullZone ? leftMasterSupplier.getAsDouble() : 0;
        double rightMaster = Math.abs(rightMasterSupplier.getAsDouble()) > kNullZone ? rightMasterSupplier.getAsDouble() : 0;
        double leftSlave = Math.abs(leftSlaveSupplier.getAsDouble()) > kNullZone ? leftSlaveSupplier.getAsDouble() : 0;
        double rightSlave = Math.abs(rightSlaveSupplier.getAsDouble()) > kNullZone ? rightSlaveSupplier.getAsDouble() : 0;
