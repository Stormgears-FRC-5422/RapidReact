    ...
    private void initCommands() {
        ...
        if (!kDiagnostic) {
            ...
            if (kUseShooter && kUseFeeder && kUseLights)
                shoot = new Shoot(feeder, shooter, lights);
            else if (kUseShooter && kUseFeeder)
                shoot = new Shoot(feeder, shooter, null);
        } else {
        ...
        if (kUseFeeder && kUsePivot && kUseIntake && kUseDrive)
            autonomous = new DoubleBallAuto(load, drive, feeder, shooter, lights, navX, vision, shooterDistanceRPSLog);
        ...
        if (kUseVision) {
            ...
            if (kUseFeeder && kUseShooter)
                shootWithVision =
                        new ShootWithVision(shooter, shoot, vision::hasTarget, vision::getDistance, shooterDistanceRPSLog);
        }
    }
        ...
        if (useDriveJoystick) {
            ...
            if (useSecondaryJoystick) {
                ...
                if (kUsePivot && kUseClimber) {
                    ...
                    buttonBoard.autoClimb.whenPressed(backbend);
