...
public class StormStruct {
  ...
  public StormStruct(
    ...
    NetworkTableEntry names_entry =
        nt_inst.getTable(base_table + "/structs/" + struct_name).getEntry("name");
    NetworkTableEntry sizes_entry =
        nt_inst.getTable(base_table + "/structs/" + struct_name).getEntry("size");
    NetworkTableEntry type_entry =
        nt_inst.getTable(base_table + "/structs/" + struct_name).getEntry("type");
  ...
  public Vector<HashMap<String, Integer>> unpack(final byte[] data_stream) {
    ...
    if (data_stream.length < 3) {
      // System.out.println("data received from Raspberry Pi is too short. Expect 3 or longer, but
      // received " + data_stream.length + " bytes");
